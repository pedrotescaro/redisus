...
function DetailSection({
  ...
  return (
    <details
      open={defaultOpen}
      className="rounded-[22px] border border-white/10 bg-surface-container p-4"
    >
...
export function AnalyzerTechnicalDrawer({
  ...
  return (
    <>
      ...
      <aside
        aria-hidden={!open}
        className={`fixed right-0 top-0 z-[80] h-screen w-full max-w-[540px] transform border-l border-white/10 bg-[#0b1220]/95 shadow-2xl backdrop-blur transition-transform ${
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
            {!analysis ? (
              <div className="rounded-[22px] border border-dashed border-white/10 bg-white/5 px-5 py-6 text-sm text-slate-300">
              ...
              <>
                  ...
                  <div className="space-y-3">
                    {tissueBreakdown.map((item) => (
                      ...
                      >
                        ...
                        {item.action ? (
                          <p className="mt-2 text-xs leading-6 text-sky-200">{item.action}</p>
                        ) : null}
                      </div>
                    ))}
                  </div>
                ...
                >
                  <div className="grid gap-4 md:grid-cols-2">
                    {analysis.visuals?.attention?.data_url ? (
                      <button
                        ...
                        className="overflow-hidden rounded-[20px] border border-white/10 bg-white/5 text-left transition-transform hover:-translate-y-0.5"
                      >
                        <img
                  ...
                  <div className="mt-4 rounded-2xl border border-white/10 bg-surface-container-low px-4 py-4">
                ...
                >
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                      <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Confianca</p>
                      <p className="mt-2 text-sm font-semibold text-white">{confidence}%</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                      <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Tempo</p>
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                      <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Contrato</p>
                      <p className="mt-2 text-sm font-semibold text-white">{analysis.contract_version}</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                      <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Margem</p>
                      <p className="mt-2 text-sm font-semibold text-white">
                        {analysis.inference.confidence_margin.toFixed(3)}
                      </p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                      <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Entropia</p>
                      <p className="mt-2 text-sm font-semibold text-white">
