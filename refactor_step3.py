...
roi_methods_definition = """import cv2
import numpy as np
import logging
...
"""
with open(os.path.join("src", "processing", "roi_segmentation.py"), "w", encoding="utf-8") as f:
    f.write(roi_methods_definition)

# Update heal_analyzer.py
with open("heal_analyzer.py", "r", encoding="utf-8") as f:
    text = f.read()

# Remove the methods from heal_analyzer.py
methods_to_remove = [
    r'    @staticmethod\n    def _exclude_surgical_background\([\s\S]*?return cleaned\n',
    r'    def _create_wound_roi_mask\([\s\S]*?return wound_mask\n',
    r'    @staticmethod\n    def _create_zone_masks\([\s\S]*?return peripheral_zone, core_zone, outer_ring\n',
    r'    @staticmethod\n    def _create_background_mask_spatial\([\s\S]*?return background_mask\n',
...
# Add imports and update calls
text = text.replace("from src.processing.wound_classifier_cv import WoundClassifierCV", "from src.processing.wound_classifier_cv import WoundClassifierCV\nfrom src.processing.roi_segmentation import ROISegmenter")

text = text.replace("self.tissue_analyzer = TissueAnalyzerCV()", "self.tissue_analyzer = TissueAnalyzerCV()\n        self.roi_segmenter = ROISegmenter()")

text = text.replace("self._create_wound_roi_mask(", "self.roi_segmenter.create_wound_roi_mask(")
text = text.replace("self._exclude_surgical_background(", "self.roi_segmenter.exclude_surgical_background(")
text = text.replace("self._create_background_mask_spatial(", "self.roi_segmenter.create_background_mask_spatial(")
text = text.replace("self._create_zone_masks(", "self.roi_segmenter.create_zone_masks(")


with open("heal_analyzer.py", "w", encoding="utf-8") as f:
    f.write(text)

print("Step 3 replacements done.")
